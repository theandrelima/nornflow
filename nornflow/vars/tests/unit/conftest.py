...
class TestHostNamespace:
    ...
    def __getattr__(self, name):
        ...
        elif hasattr(self.mock_host, "get"):
            value = self.mock_host.get(name)
            if value is not None:
                return value
        # If attribute doesn't exist, raise an error like the real HostNamespace would
        raise AttributeError(f"Host attribute or data key '{name}' not found for host '{self.mock_host.name}'")
...
def setup_manager(vars_dir, workflows_dir, mock_host_proxy, mock_host):
    ...
    manager = NornFlowVariablesManager(
        ...
        inline_workflow_vars={"workflow_var": "workflow_value", "override_var": "workflow_value", "backup_type": "full"},
