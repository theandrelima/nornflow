        ...
        try:
            ...
            if task.name == "set":
                ...
                if task.params: # Ensure params exist to avoid errors if 'set' is called with no args
                    ...
                    logger.warning(f"Task 'set' for host '{host.name}' called with no arguments. No variables will be set.")
            else:
            ...
            logger.error(f"Error processing variables for task '{task.name}' on host '{host.name}': {e}", exc_info=True)
