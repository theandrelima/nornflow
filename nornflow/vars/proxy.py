...
class NornirHostProxy:
    ...
    def current_host_name(self, host_name: str | None) -> None:
        ...
        if not self._nornir:
            logger.warning("NornirHostProxy: Nornir instance not set. Cannot look up host by name '%s'. Clearing current_host.", host_name)
            self.current_host = None
            return
        ...
        if host_name in self._nornir.inventory.hosts:
            ...
            logger.warning("NornirHostProxy: Host '%s' not found in Nornir inventory. Clearing current_host.", host_name)
            self.current_host = None
    ...
    def _get_host_value(self, name: str) -> Any:
        ...
        Raises:
            HostContextError: If `self._current_host` is not set (safeguard, should be caught by `__getattr__`).
