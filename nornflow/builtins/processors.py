...
class NornFlowFailureStrategyProcessor(Processor):
    ...
    def task_started(self, task: Task) -> None:
        ...
        if self.failure_strategy == FailureStrategy.RUN_ALL and self.nornir:
            ...
            with output_lock:
                print(f"{Fore.RED}{Style.BRIGHT}Execution is halting because a task failed and FAIL_FAST is enabled.{Style.RESET_ALL}")
            ...
            if self.failure_strategy == FailureStrategy.FAIL_FAST and not self.fail_fast_triggered:
                ...
                if self.nornir:
                    with output_lock:
                        ...
                        print(f"{Fore.RED}NOTE: Tasks already started will continue to completion.{Style.RESET_ALL}")
                        print()
