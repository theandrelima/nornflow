...
def settings_callback(ctx: typer.Context, settings: Optional[str] = None):
    """
    ...
    if nornflow_config_file:
        typer.secho(
            ...
            "for its settings file.", fg=typer.colors.MAGENTA, bold=True
        )
        typer.secho(
            "Unset it to make use of '--settings' option, or to fallback to a default 'nornflow.yaml' file.", fg=typer.colors.MAGENTA, bold=True
        )
        typer.secho(f"NORNFLOW_CONFIG_FILE='{nornflow_config_file}'\n", fg=typer.colors.BRIGHT_YELLOW, bold=True)
...
def main(
    ctx: typer.Context,
    settings: Optional[str] = typer.Option(None, "--settings", "-s", help="Specify a path to a custom settings file."),
):
