...
class Catalog(ABC, dict[str, Any]):
    ...
    def get_all_items_info(self, include_name: bool = False) -> dict[str, dict[str, Any]]:
        ...
        """
        return {name: self.get_item_info(name, include_name) for name in self if self.get_item_info(name) is not None}
    ...
    def get_builtin_items(self) -> dict[str, Any]:
        ...
        """
        return {
            name: self[name] for name in self
            if self.sources.get(name, {}).get("is_builtin", False)
        }
    ...
    def get_custom_items(self) -> dict[str, Any]:
        ...
        return {
            name: self[name] for name in self
            if not self.sources.get(name, {}).get("is_builtin", False)
        }
...
class PythonEntityCatalog(Catalog):
    ...
    def register(self, name: str, item: Any, module_path: str | None = None, 
                module_name: str | None = None, **kwargs) -> Any:
    ...
    def register_from_module(self, module: Any, predicate: Callable[[Any], bool] | None = None, 
                           transform_item: Callable[[Any], Any] | None = None) -> int:
    ...
    def discover_items_in_dir(
        ...
        **kwargs
        ...
        Raises:
            ...
            CoreError: If module import fails.
        """
        return super().discover_items_in_dir(dir_path, predicate=predicate, transform_item=transform_item, **kwargs)
...
class FileCatalog(Catalog):
    ...
    def get_by_extension(self, extension: str) -> dict[str, Path]:
        ...
        return {
            name: path for name, path in self.items()
